...
ipcMain.handle('process:start', async (event, config) => {
    ...
    try {
        // validate dirs
        if (!fs.existsSync(input1.path) || !fs.existsSync(input2.path) || !fs.existsSync(input3.path)) {
        ...
        if (loop) {
            ...
            if (neededDuration <= 0) {
                log("Warning: Input 3 is longer than target duration. Only Input 3 will be used.", 'warn');
                ...
                while (filledDuration < neededDuration) {
                    ...
                    log(`Iteration ${iteration}: Current Duration ${filledDuration.toFixed(2)}s / ${neededDuration.toFixed(2)}s`);
                    iteration++;

                    // Safety break
                    if (iteration > 1000) {
                        log("Safety limit reached (1000 iterations). Stopping loop.", 'warn');
                        break;
                    }
                }
        ...
        const listPath = path.join(app.getPath('temp'), `concat_list_${Date.now()}.txt`);
        // Format for concat demuxer: file 'path'
        const listContent = finalList.map(f => `file '${f.replace(/'/g, "'\\''")}'`).join('\n');
        fs.writeFileSync(listPath, listContent);

        const outputFileName = `output_${Date.now()}.wav`;
        const outputPath = path.join(output, outputFileName);

        log(`Processing merge... ${finalList.length} files. Output target: ${outputPath}`);
