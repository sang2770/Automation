    ...
    setupIpcHandlers() {
        ...
        ipcMain.handle('load-gpm-profiles', async () => {
            try {
                ...
                const response = await axios.get('http://127.0.0.1:19995/api/v3/profiles', {
                    timeout: 5000
                });
                ...
                return {
                    success: false,
                    error: 'GPM not running or not accessible. Please ensure GPMLogin is running on port 19995.'
                };
            }
        });
        ...
        ipcMain.handle('start-automation', async (event, config) => {
            ...
            try {
                this.isRunning = true;
                this.workerProcess = spawn('node', [path.join(__dirname, 'youtube-worker.js')], {
