...
class YouTubeAutomationUI {
  ...
  updateUIFromConfig() {
    ...
    document.getElementById("actionDelay").value =
      this.config.settings?.delayBetweenActions || 2000;
    document.getElementById("randomMethod").checked =
      this.config.settings?.randomMethod !== false;

    // Update proxy settings
    document.getElementById("enableProxy").checked =
      this.config.proxy?.enabled || false;
    document.getElementById("proxyList").value =
      this.config.proxy?.list?.join("\n") || "";
  ...
  updateConfigFromUI() {
    this.config.maxThreads = parseInt(
  ...
  renderLinks() {
    ...
    if (!this.config.links || this.config.links.length === 0) {
      linksList.innerHTML =
        '<div class="list-group-item text-muted">No links configured</div>';
      return;
    }

    let html = "";
    this.config.links.forEach((link, index) => {
      const keywords = Array.isArray(link.keywords)
        ? link.keywords.join(", ")
        : link.keywords || "";
      ...
      // Show ad option and watch duration info
      const adOption = link.adOption || "click";
      const adText = adOption === "click" ? "Click ads" : "Skip ads";
      const watchDurationText =
        adOption === "skip" && link.watchDuration
          ? ` (${link.watchDuration}s)`
          : "";
  ...
  showLinkModal(linkData = null) {
    ...
    if (linkData) {
      ...
      document.getElementById("linkKeywords").value = Array.isArray(
        ...
        ? linkData.keywords.join(", ")
        : linkData.keywords || "";
      document.getElementById("linkEnabled").checked =
        linkData.enabled !== false;

      // Set ad option
      const adOption = linkData.adOption || "click";
      if (adOption === "click") {
        document.getElementById("clickAds").checked = true;
      } else {
        document.getElementById("skipAds").checked = true;
      }

      // Set watch duration
      if (linkData.watchDuration) {
        document.getElementById("watchDuration").value = linkData.watchDuration;
      } else {
        document.getElementById("watchDuration").value = 30;
      }

      // Trigger toggle to show/hide watch duration
      const skipAds = document.getElementById("skipAds");
      if (skipAds.checked) {
        document.getElementById("watchDurationGroup").style.display = "block";
      } else {
        document.getElementById("watchDurationGroup").style.display = "none";
      }
  ...
  async saveLink() {
    ...
    // Get watch duration (only needed if skip ads is selected)
    ...
    const existingIndex = this.config.links.findIndex(
      (link) => link.id === this.currentLinkId,
    );
  ...
  editLink(linkId) {
    const link = this.config.links.find(
      (l) => l.id === linkId || this.config.links.indexOf(l) === linkId,
    );
    if (link) {
  ...
  async deleteLink(linkId) {
    if (confirm("Are you sure you want to delete this link?")) {
      this.config.links = this.config.links.filter(
        (l, index) => l.id !== linkId && index !== linkId,
      );
      this.renderLinks();
